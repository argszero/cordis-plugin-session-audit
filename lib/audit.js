 ...
 */
import { ZstdStructureError, decodeZstdArtifact, looksLikeZstd, } from './zstd.js';
import { encodeSegment, expectedProjectDirName, grandparentName, parentName, pathSegments, } from './paths.js';
 ...
 */
function checkPathIdentity(audit) {
    const segments = pathSegments(audit.path);
    if (segments.length < 3)
        return; // no project/session directory to compare against
    if (audit.formatVersion < 0)
        return; // no readable header: nothing to compare against
 ...
 */
export function auditSessionArtifact(bytes, path) {
    ...
    if (!zstdNamed && !looksLikeZstd(bytes))
        return auditSessionLog(bytes, path);
    let decoded;
    try {
        ...
        return {
            ...
            findings: [{
                    ...
                    severity: 'error',
                    detail: structural
                        ? `${error.message} — the artifact listing aborts on this file, which `
                        ...
                        : `${error.message} — the container structure is intact, so the frames can `
                            + 'be located, but their content could not be decoded.',
                    at: structural ? error.offset : 0,
                }],
        };
    }
