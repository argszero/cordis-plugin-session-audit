...
export function looksLikeZstd(bytes) {
...
export function zstdDecodeSupported() {
 ...
 */
export function decodeZstdArtifact(bytes) {
    const scan = scanZstdFrames(bytes);
    const sync = zstdSync();
    if (sync === undefined) {
        throw new Error('this Node has no zstdDecompressSync, so the container could not be read '
    ...
    for (const frame of scan.frames) {
        ...
        try {
            ...
            throw new Error(`Zstandard frame at byte ${frame.start} failed validation `
                + `(checksum or entropy decode): ${error.message}`, { cause: error });
        }
    ...
    if (scan.tornStart !== undefined) {
        ...
        try {
            const flush = nodeZlib.constants.ZSTD_e_flush ?? 1;
            const recovered = sync(tail, { finishFlush: flush });
            if (recovered.length > 0)
                tornText = recovered.toString('utf8');
        }
        catch {
