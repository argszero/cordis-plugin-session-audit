...
/** A normalized event from a packed chunk row (expanded to its member events). */
type AuditEvent = ParsedEvent
 ...
 */
function decodeLine(line: string, lineNo: number): ParsedEvent[] | null {
  let raw: unknown
  try {
    raw = JSON.parse(line)
  } catch {
  ...
  if (type === 'text-chunks' || type === 'reasoning-chunks' || type === 'tool-call-chunks') {
    const seq0 = rec['seq0'] as number | undefined
    const time0 = rec['time0'] as number | undefined
    const data = rec['data'] as Record<string, unknown> | undefined
    if (!Number.isSafeInteger(seq0) || !Number.isSafeInteger(time0) || !data) return null
    const turn = data['turn'] as number | undefined
    const step = data['step'] as number | undefined
    const dt = data['dt'] as number[] | undefined
    const members: string[] = type === 'tool-call-chunks'
      ? (data['args'] as string[] | undefined) ?? []
      : (data['texts'] as string[] | undefined) ?? []
    const runKind = type === 'text-chunks' ? 'text-delta'
      : type === 'reasoning-chunks' ? 'reasoning-delta'
      : 'tool-call-delta'
    const callId = type === 'tool-call-chunks' ? (data['id'] as string | undefined) : undefined
    const callName = type === 'tool-call-chunks' ? (data['name'] as string | undefined) : undefined
    if (!Array.isArray(dt)) return null
    const events: ParsedEvent[] = []
    let seq = seq0 as number
    let time = time0 as number
    for (let k = 0; k < members.length; k++) {
      const chunkData: Record<string, unknown> = { block: data['index'], delta: runKind }
      if (type === 'tool-call-chunks') {
        chunkData['callId'] = callId
        if (callName !== undefined) chunkData['name'] = callName
  ...
  const seq = rec['seq'] as number | undefined
  const time = rec['time'] as number | undefined
  ...
  return [{
    ...
    time: time as number,
    data: rec['data'] ?? {},
    ...(rec['ignorable'] === true ? { ignorable: true as const } : {}),
  }]
}
...
function decodeHeader(line: string): { id: string; version: number; seedLength: number } | null {
  ...
  if (typeof raw !== 'object' || raw === null) return null
  const rec = raw as Record<string, unknown>
  if (rec['type'] !== 'session') return null
  const id = rec['id']
 ...
 */
export function auditSessionLog(bytes: Buffer, path: string): SessionAudit {
  const text = bytes.toString('utf8')
  const lines = text.split('\n')
  const headerLine = lines[0] ?? ''
  const header = decodeHeader(headerLine)

  const result: SessionAudit = {
  ...
  const callIds = new Map<string, { seq: number }>()
  ...
  for (const ev of events) {
    ...
    if (ev.type === 'tool/call') {
      ...
      const block = (Array.isArray(content) ? (content[0] ?? {}) : {}) as Record<string, unknown>
      const callId = (block['toolCallId'] ?? d['callId']) as string | undefined
      if (callId === undefined || callId === '') {
        result.findings.push({
  ...
  for (const ev of events) {
    const d = ev.data as Record<string, unknown>
    if (ev.type === 'turn/start') openTurn = d['turn'] as number ?? null
    else if (ev.type === 'turn/end') {
      closedTurn = d['turn'] as number ?? -1
      if (d['turn'] === openTurn) openTurn = null
    }
  }
  if (openTurn !== null) {
    result.findings.push({
